...
CONFIG_LOCALES_PATH="src/config/locales.json"

mkdir -p "$LOCALES_DIR_NAME"

docker run --rm -v "$LOCALES_DIR_NAME:$TEMP_DIR" -w "$TEMP_DIR" --network "$NETWORK_NAME" -e LOCALES_PATH="$CONFIG_LOCALES_PATH" -e PRIVATE_STRAPI_URL="$STRAPI_URL" -e SCRIPT_PATH="$SCRIPT_PATH" -e UID="$USER_ID" -e GID="$GROUP_ID" node:22-alpine sh -c '
